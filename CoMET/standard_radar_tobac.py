...
def standard_radar_tobac_feature_id(cube, CONFIG):
    ...
    if ("height" in inCONFIG["standard_radar"]["tobac"]["feature_id"]):
        ...
        if (inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"] == None or type(inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"] ) == str or type(CONFIG["standard_radar"]["tobac"]["feature_id"]["height"] ) == bool):
            raise Exception(f"""!=====Segmentation Height Out of Bounds. You Entered: {inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"] .lower()}=====!""")
            return
        if (inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"]  > cube.coord("altitude").points.max() or inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"]  < cube.coord("altitude").points.min()):
            raise Exception(f"""!=====Segmentation Height Out of Bounds. You Entered: {inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"] .lower()}=====!""")
            return
        ...
        height_index = find_nearest(cube.coord("altitude").points, inCONFIG["standard_radar"]["tobac"]["feature_id"]["height"])
    ...
    if (cube.coord("altitude").shape[0] == 1):
        ...
        radar_features = tobac.feature_detection.feature_detection_multithreshold(feat_cube[:,0], dxy=dxy, **inCONFIG["standard_radar"]["tobac"]["feature_id"])
        
    else:
        
        # Perform tobac feature identification and then convert to a geodataframe before returning
        radar_features = tobac.feature_detection.feature_detection_multithreshold(feat_cube, dxy=dxy, **inCONFIG["standard_radar"]["tobac"]["feature_id"])
    ...
    radar_geopd = gpd.GeoDataFrame(
        radar_features, geometry = gpd.points_from_xy(radar_features.longitude, radar_features.latitude), crs="EPSG:4326"
    )
...
def standard_radar_tobac_linking(cube, radar_features, CONFIG):
    ...
    if (cube.coord("altitude").shape[0] == 1):
        ...
        radar_tracks = tobac.linking_trackpy(radar_features,cube[:,0],dt=dt,dxy=dxy,**CONFIG["standard_radar"]["tobac"]["linking"])
        
    else:
        
        # Do tracking then convert output dataframe to a geodataframe
        radar_tracks = tobac.linking_trackpy(radar_features,cube,dt=dt,dxy=dxy,vertical_coord="altitude",**CONFIG["standard_radar"]["tobac"]["linking"])
    ...
    radar_geopd_tracks = gpd.GeoDataFrame(
        radar_tracks, geometry = gpd.points_from_xy(radar_tracks.longitude, radar_tracks.latitude), crs="EPSG:4326"
    )
...
"""
def standard_radar_tobac_segmentation(cube, radar_features, segmentation_type, CONFIG, segmentation_height = None):
    import tobac
    import xarray as xr
    from copy import deepcopy
    ...
    if (segmentation_type.lower() == "2d"):
        ...
        if ("height" in inCONFIG["standard_radar"]["tobac"]["segmentation_2d"]): del inCONFIG["standard_radar"]["tobac"]["segmentation_2d"]["height"]
        ...
        segment_cube, segment_features = tobac.segmentation_2D(radar_features, seg_cube, dxy=dxy, **inCONFIG["standard_radar"]["tobac"]["segmentation_2d"])
        ...
        segment_cube, segment_features = tobac.segmentation_3D(radar_features, cube, dxy=dxy, **inCONFIG["standard_radar"]["tobac"]["segmentation_3d"])
