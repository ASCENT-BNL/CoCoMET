...
# =============================================================================
# Loads in pre-gridded radar data which follows the radar standardization set out in the CoMET-UDAF Section #.#.
# =============================================================================
...
def standard_radar_load_netcdf_iris(path_to_files, tracking_var, CONFIG):
    ...
    if (tracking_var.lower() == "dbz"):
        ...
        radar_xarray = xr.open_mfdataset(path_to_files, concat_dim="time", combine="nested").reflectivity
        ...
        if ("standard_radar" in CONFIG):
            ...
            if ("bounds" in CONFIG["standard_radar"]):
                ...
                mask_lon = (radar_xarray.lon >= CONFIG["standard_radar"]["bounds"][0]) & (radar_xarray.lon <= CONFIG["standard_radar"]["bounds"][1])
                mask_lat = (radar_xarray.lat >= CONFIG["standard_radar"]["bounds"][2]) & (radar_xarray.lat <= CONFIG["standard_radar"]["bounds"][3])
        ...
        radar_xarray = radar_xarray.assign_coords(time = ("time", (radar_xarray.time.values-first_time).astype("timedelta64[m]").astype(float)))
        radar_xarray["time"] = radar_xarray.time.assign_attrs({"standard_name": "time", "long_name": f"minutes since {first_time}", "units": f"minutes since {first_time}"})
        ...
        radar_xarray["altitude"] = radar_xarray.altitude.assign_attrs({"standard_name": "altitude", "units": "m"})
