...
def goes_load_netcdf_iris(path_to_files, tracking_var, CONFIG):
    ...
    if (tracking_var.lower() == 'tb'):
        ...
        goes_xarray = xr.open_mfdataset(path_to_files, coords='all', concat_dim='t', combine='nested')
        ...
        if ('goes' in CONFIG):
            ...
            if ('bounds' in CONFIG['goes']):
                ...
                mask_lon = (goes_xarray.lon >= CONFIG['goes']['bounds'][0]) & (goes_xarray.lon <= CONFIG['goes']['bounds'][1])
                mask_lat = (goes_xarray.lat >= CONFIG['goes']['bounds'][2]) & (goes_xarray.lat <= CONFIG['goes']['bounds'][3])
