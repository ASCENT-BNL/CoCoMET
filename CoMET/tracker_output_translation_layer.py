...
"""
def bulk_moaap_to_UDAF(mask, projection_x_coords, projection_y_coords, convert_type="cloud"):
    import numpy as np
    import pandas as pd
    import xarray as xr
    from tqdm import tqdm
    ...
    if (convert_type.lower() == "cloud"):
        ...
        if ("BT_Objects" not in mask): return None
        
        mask_field = mask.BT_Objects.values
        
    elif (convert_type.lower() == "mcs"):
        
        if ("MCS_Tb_Objects" not in mask): return None
    ...
    for ii in tqdm(range(mask_field.shape[0]), desc="=====Converting MOAAP to UDAF=====", total=mask_field.shape[0]):
        ...
        for cell_id in unique_cells:
            ...
            if (int(cell_id)-1 not in lifetime_dict): 
                ...
                lifetime_dict[int(cell_id)-1] = { "frame": [ii], "lifetime": [pd.Timedelta(0)] }
                lifetimes.append(pd.Timedelta(0))
                
            else:
                
                new_time = lifetime_dict[int(cell_id)-1]["lifetime"][-1] + (pd.Timedelta(minutes=dt) * (ii - lifetime_dict[int(cell_id)-1]["frame"][-1]))
    ...
    for row in tqdm(temp_linking.iterrows(), desc="=====Processing MOAAP Cell Lifetimes====", total=temp_linking.shape[0]):
        
        cell_max_life = temp_linking.query("cell_id==@row[1].cell_id").lifetime.values.max()
    ...
    UDAF_features = pd.DataFrame(data={"frame": frames, "time": times, "feature_id": feature_ids[1:],
                                      "south_north": south_norths, "west_east": west_easts, "up_down": np.repeat(np.nan, len(frames)),
                                      "latitude": lats, "longitude": lons,
                                      "projection_x": new_proj_x, "projection_y": new_proj_y,
                                      "altitude": np.repeat(np.nan, len(frames)) })
    
    UDAF_features = gpd.GeoDataFrame(
        UDAF_features, geometry = gpd.points_from_xy(UDAF_features.longitude, UDAF_features.latitude), crs="EPSG:4326"
    )
    
    
    UDAF_linking = pd.DataFrame(data={"frame": frames, "time": times, "lifetime": lifetimes, "lifetime_percent": lifetime_percents, 
                                      "feature_id": feature_ids[1:], "cell_id": cell_ids,
                                      "south_north": south_norths, "west_east": west_easts, "up_down": np.repeat(np.nan, len(frames)),
                                      "latitude": lats, "longitude": lons,
                                      "projection_x": new_proj_x, "projection_y": new_proj_y,
                                      "altitude": np.repeat(np.nan, len(frames)) })
    ...
    UDAF_linking = gpd.GeoDataFrame(
        UDAF_linking, geometry = gpd.points_from_xy(UDAF_linking.longitude, UDAF_linking.latitude), crs="EPSG:4326"
    )
    ...
    UDAF_segmentation_2d = xr.Dataset(
        ...
        data_vars=dict(
            Feature_Segmentation=(["time", "south_north", "west_east"], segment_feature_mask),
            Cell_Segmentation=(["time", "south_north", "west_east"], segment_cell_mask)
        ),
        attrs=dict(
            description="Mask for tracked objects from MOAAP"
        )
    )
