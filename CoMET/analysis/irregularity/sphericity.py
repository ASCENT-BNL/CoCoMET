...
def sphericity(
    ...
    **args : dict
    ...
    feature_ids = []
    cell_ids = []
    for row in tqdm(surface_area_df.itertuples(), desc='=====Calculating Cell Sphericities=====', total=len(surface_area_df)):
        i = row[0]
        frames.append(row[1])
        ...
        surface_area = row[4] #this is assuming both the surface area and volume are in units of km
        volume = volume_df.iloc[i]

        if surface_area == 0 or surface_area != surface_area: #check for 0 or NaN values
            sphericities.append(np.NaN)
            continue
        
        if volume == 0 or volume != volume: #check for 0 or NaN values
            sphericities.append(np.NaN)
            continue
    ...
    sphericity_df = pd.DataFrame(
        {'frame': frames, 'feature_id': feature_ids, 'cell_id': cell_ids, 'sphericity': sphericities}
    )
