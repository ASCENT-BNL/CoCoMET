...
def wrf_load_netcdf_iris(filepath, tracking_var, CONFIG):
    import numpy as np
    import xarray as xr
    from .wrf_calculate_products import wrf_calculate_reflectivity, wrf_calculate_agl_z, wrf_calculate_wa, wrf_calculate_brightness_temp, wrf_calculate_precip_rate
    ...
    proj_y_values = wrf_xarray.DY * (np.arange(0, wrf_xarray.south_north.shape[0]) + 0.5)
    ...
    if (tracking_var.lower() == "dbz"):
        ...
        wrf_xarray["PR"] = (["Time","south_north","west_east"], wrf_calculate_precip_rate(wrf_xarray))
