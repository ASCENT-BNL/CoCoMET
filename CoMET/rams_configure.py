def configure_rams(dataset0, path_to_header, CONFIG={'verbose': False}):
    """
    ...
    if len(listing) > 1:
        ...
        init_date = f'minutes since {year}-{month}-{date} {int(itime[:2]) + hour}:{int(itime[2:]) + minute}:{second}'
        dataset.attrs["init_date"] = init_date
