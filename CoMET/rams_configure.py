def configure_rams(dataset0, path_to_header, CONFIG = None):
    """
    Serves to set up proper attributes, coordinates, and dimensions so that the RAMS data set is compatible with CoMET
    Inputs:
    ...
    if CONFIG is None: CONFIG = {'verbose' : False, 'rams' : {}}
    ...
    ListOfNeededVariables = ['GLAT', 'GLON', 'TOPT', # lat, lon, and topographical height
                             'PI', 'THETA', # pressure and temperature var
                             'RV', 'RCP', 'RDP', # mixing ratio (dbz calculation)
                             'RRP', 'RPP', 'RSP', 
                             'RAP', 'RGP', 'RHP', 
                             'CCP', 'CDP', 'CRP', # concentration numbers (dbz calculation)
                             'CPP', 'CSP', 'CAP',
                             'CGP', 'CHP',
                             'PCPVR', 'PCPVS', 'PCPVA', # 3D precipitation rates
                             'PCPVG', 'PCPVH', 'PCPVD',
    ...
    dataset = xr.Dataset({})
    if CONFIG['verbose']:
        for var in tqdm(ListOfNeededVariables,
            ...
            if len(dims) == 3:
                ...
                dataset[var].chunk(dataset0[var][:, 1:, 1:].chunksizes) # rechunk the variables
            elif len(dims) == 4:
                dataset[var] = ([*dims], dataset0[var].data[:, 1:, 1:, 1:])
                dataset[var].chunk(dataset0[var][:, 1:, 1:, 1:].chunksizes) # rechunk the variables
    else:
    ...
    dataset = dataset.rename({'phony_dim_0' : 'south_north', 'phony_dim_1' : 'west_east', 'phony_dim_2' : 'bottom_top'})#, 'phony_dim_3' : 'np', 'phony_dim_4' : 'nzs', 'phony_dim_5' : 'local_topography_height'})
    pbar.update()
    pbar.refresh()

    # define the coordinates
    dataset = dataset.assign_coords(Times=(["Time"], np.arange(len(dataset['Time']))))
    dataset = dataset.assign_coords(GLAT=(["Time", "south_north", "west_east"], dataset["GLAT"].data))
    dataset = dataset.assign_coords(GLON=(["Time", "south_north", "west_east"], dataset["GLON"].data))
    pbar.update()
    pbar.refresh()
    ...
    if len(listing) > 1:
        ...
        datetime_start_date = datetime.strptime(init_date_str_unformatted, '%Y-%m-%d %H:%M:%S')
        init_date_str_formatted = datetime.strftime(datetime_start_date, '%Y-%m-%d %H:%M:%S')
    ...
    headerfile = np.loadtxt(firstHeaderStr, dtype=str, skiprows=findWhereColsChange(firstHeaderStr))
    DX = float(headerfile[np.where(headerfile == '__xmn03')[0][0]+3]) - float(headerfile[np.where(headerfile == '__xmn03')[0][0]+2])
    DY = float(headerfile[np.where(headerfile == '__ymn03')[0][0]+3]) - float(headerfile[np.where(headerfile == '__ymn03')[0][0]+2])
    dataset.attrs["DY"] = DY
    dataset.attrs["DX"] = DX

    # calculate projection coordinates using linear projection constants
    proj_y_values = dataset.DY * (
        np.arange(0, dataset.south_north.shape[0]) + 0.5
    )
    ...
    __zmn03 = np.array(headerfile[startunstag:stopunstag], dtype=float)[1:] # the first data point is unphysical
    ...
    if 'moaap' in CONFIG['rams']:
        ...
        for t in tqdm(range(TOPT.shape[0]),
                    desc='=====Calculating Geopotential for MOAAP Tracking=====',
                    total=TOPT.shape[0]):
            alt = np.array(np.expand_dims(dataset['altitudes'].values, (1, 2)), dtype=float)
            geopt[t, :, :, :] = (np.array(np.expand_dims(TOPT, 1)[t, :, :, :], dtype=float) + alt) * 9.81
        geopt = xr.DataArray(geopt, dims=['Time', 'bottom_top', 'south_north', 'west_east'])
        geopt.chunk(dataset["THETA"].chunksizes)
        dataset['geopt'] = (['Time', 'bottom_top', 'south_north', 'west_east'], geopt.data)
