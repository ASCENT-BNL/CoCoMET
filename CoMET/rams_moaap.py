...
def rams_moaap(rams_xarray, CONFIG):
    """
    Inputs:
    ...
    v_winds_850 = (
        rams_xarray["VC"][:, height_idx_850, :, :]).values
    u_winds_850 = (
        rams_xarray["UC"][:, height_idx_850, :, :]).values
    v_winds_200 = (
        rams_xarray["VC"][:, height_idx_200, :, :]).values
    u_winds_200 = (
        rams_xarray["UC"][:, height_idx_200, :, :]).values
    ...
    mr = (
        ...
        + rams_xarray.RHP
        + rams_xarray.RAP
    )[:, height_idx_850] * 1000 # kg / kg -> g / kg
    ...
    mask_file = xr.open_mfdataset(
        output_filepath, coords="all", concat_dim="time", combine="nested", decode_time=False
    )
